    ...
    <section id="projects" className="py-24 px-6" ref={ref}>
      <div className={`max-w-6xl mx-auto transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
        <div className="mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-[var(--text-primary)] mb-3 tracking-tight">Projects</h2>
        ...
        <div className="flex gap-3 mb-10">
          {(['all','featured'] as const).map(tab => (
            <button key={tab} onClick={() => setFilter(tab)}
              ...
              style={filter === tab
                ? { background: '#2563eb', color: '#fff', boxShadow: '0 0 15px rgba(37,99,235,0.3)' }
                : { border: '2px solid #2563eb', color: '#2563eb', background: 'transparent' }}
              onMouseEnter={e => { if (filter !== tab) { e.currentTarget.style.background = '#2563eb'; e.currentTarget.style.color = '#fff' }}}
              onMouseLeave={e => { if (filter !== tab) { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = '#2563eb' }}}>
              {tab}
            </button>
          ))}
        </div>
              ...
              <div className="card h-full p-6 group flex flex-col">
                <div className="flex items-center justify-between mb-4">
                  ...
                  {p.featured && (
                    <span className="px-2 py-0.5 text-xs font-mono rounded border"
                      style={{ color: '#2563eb', background: 'rgba(37,99,235,0.1)', borderColor: 'rgba(37,99,235,0.2)' }}>
                      ★ featured
                    </span>
                  )}
                </div>

                <h3 className="text-[var(--text-primary)] font-bold text-lg mb-2 group-hover:text-blue-bright transition-colors">{p.title}</h3>
                ...
                <div className="flex gap-3">
                  <a href={p.github} target="_blank" rel="noreferrer"
                    className="flex items-center gap-2 px-3 py-2 text-xs font-mono rounded-lg transition-all"
